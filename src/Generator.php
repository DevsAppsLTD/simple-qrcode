...
class Generator
{
	...
	/**
	 * Holds the selected backend (imagick|gd).
	 *
	 * @var string|null
	 */
	...
	public function backend(string $backend): self
	{
		...
		if (! in_array($backend, ['gd', 'imagick'])) {
			throw new InvalidArgumentException("\$backend must be 'gd' or 'imagick'. {$backend} is not valid.");
		}
	...
	public function getFormatter(): ImageBackEndInterface
	{
		if ($this->format === 'png') {
			if ($this->backend === 'gd') {
				return new SvgImageBackEnd; // fallback neutre
			}
