...
class GeneratorTest extends TestCase
{
	...
	public function test_it_can_generate_qrcode_with_gd()
	{
		...
		$this->assertStringContainsString("\x89PNG", $qr, 'Should contain PNG header');
	}

	public function test_it_can_generate_qrcode_with_imagick()
	{
		...
		$qr = (new Generator)
			...
			->size(300)
			->errorCorrection('H')
			->generate('Hello Imagick!');

		$this->assertNotEmpty($qr);
		$this->assertStringContainsString("\x89PNG", $qr, 'Should contain PNG header');
	}

	public function test_it_falls_back_when_backend_not_set()
	{
		$qr = (new Generator)
			->format('png')
			->size(200)
			->errorCorrection('M')
			->generate('Auto backend test');
